...
DEFAULT_MODEL_DIR = PROJECT_DIR / "models" / "gemma-2-9b-it"
...
def create_gemma2_model_with_lora(args, tokenizer):
...
class BestTestAccCallback(TrainerCallback):
    """
    Save only one global LoRA adapter: the checkpoint that obtains the highest
    test ACC across all Hyperopt trials, folds, and evaluation epochs.
    """
    ...
    def _save_global_best_model(self, state, metrics: dict, test_acc: float):
        ...
        pred_df = save_predictions(
            ...
            tokenized_dataset=self.tokenized_test,
            raw_df=self.raw_test_df,
...
def save_predictions(trainer, tokenized_dataset, raw_df, output_path):
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default=str(DEFAULT_MODEL_DIR))
    parser.add_argument(
        "--model_repo_id",
        type=str,
    ...
    parser.add_argument("--output_dir", type=str, default="gemma_prompt_opt_output")
    parser.add_argument("--best_lora_name", type=str, default="Best_lora_prompt")
    ...
    # Prompt version enabled by default; --no_prompt is kept only for ablation.
    ...
    parser.add_argument(
        "--prompt_text",
        type=str,
        default="Determine the strength of Escherichia coli promoters.",
    )
    ...
    best_lora_dir = os.path.join(args.output_dir, args.best_lora_name)
    ...
    df.to_csv(
        os.path.join(args.output_dir, "all_training_data.csv"),
        index=False,
    )
    ...
    def tokenize_batch(batch):
        sequences = batch[args.sequence_col]

        if args.use_prompt:
            texts = [
                args.prompt_text.strip() + " " + str(seq)
                for seq in sequences
            ]
        else:
            texts = [str(seq) for seq in sequences]
    ...
    def objective(sampled_params):
        ...
        for fold_idx, (train_index, test_index) in enumerate(cv_splits, start=1):
            ...
            model = create_gemma2_model_with_lora(trial_args, tokenizer)
    ...
    print("\n" + "=" * 90)
    print("Prompt Hyperopt TPE search finished.")
    print("=" * 90)
