...
class EpochStepDecayBestTestTrainer(Trainer):
    ...
    def _save_best_test_outputs(self, metrics):
        ...
        if not isinstance(self.eval_dataset, dict) or "test" not in self.eval_dataset:
...
def main():
    ...
    # 保险处理：Mistral/GenomeOcean 不使用 token_type_ids
    ...
    def tokenize_batch(batch):
        ...
        # 核心修复：GenomeOcean / Mistral 不接受 token_type_ids
    ...
    for fold_idx, (train_index, test_index) in enumerate(skf.split(df, labels), start=1):
        ...
        model = create_genomeocean_model_with_lora(args, tokenizer)
        ...
        training_args = TrainingArguments(
            ...
            # 核心修改：避免 tokenizer fork warning 和潜在死锁
    ...
    print("\n" + "=" * 90)
    print("GenomeOcean five-fold cross-validation finished.")
    print("=" * 90)
